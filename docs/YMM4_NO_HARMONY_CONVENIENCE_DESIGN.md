...
作成日: 2026-09-23
状態: **ADOPTED WORK POLICY / IMPLEMENTATION BASELINE**  
この文書は今後の便利機能Full再現作業の基準方針。追加機能の実装・実機PASSそのものを意味しない。
...
この文書を、P4以降の**便利機能Full再現の実装基準**として採用する。

優先順位は次の通り。

1. 既存のP0-P3 freeze / exact-host evidence。
2. 本書の所有権・単純化・実装順の方針。
3. 個別便利機能の実装詳細。

個別実装は、本書と同等以上の保守性・単純性・検証可能性を示せる場合に限って構造変更してよい。
単に本家と同じクラス構成だから、あるいは機能追加が速いからという理由では、本書の境界を崩さない。

本家については**機能集合を再現対象**とするが、内部構造・Harmony patch方式・別構造推定器・独自Undo stackは再現対象にしない。

### 変更時の原則

- Coreの意味を変えない。
- 新しい状態の正本を増やさない。
- YMM4 mutation / Undo / persistence / FoldMap / navigationの所有者を増やさない。
- 既存の実証済み経路で表現できる機能のために、新しいhost依存層を作らない。
- 便利機能ごとの常駐Service / Interface / Factoryを機械的に増やさない。
- 変更が必要な場合は、既存案より高い水準の保守性と失敗時安全性を示す。
