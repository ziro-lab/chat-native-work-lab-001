#!/usr/bin/env python3
"""Rank loop-phase candidates using Beat This detected grids plus lightweight chroma context."""
...
def build_variant_features(path: Path, beats: list[float], downbeats: list[float], duration: float) -> dict:
...
def evaluate_query(candidates: list[dict]) -> dict:
    ...
    pos_id = positives[0]["id"]
    rank = next(i + 1 for i, c in enumerate(ranked) if c["id"] == pos_id)
    ps = positives[0]["score"]
    neg = [c["score"] for c in candidates if c["label"] != "positive"]
    pairwise = sum(1.0 if ps > n else 0.5 if ps == n else 0.0 for n in neg) / len(neg)
    return {
        "covered": True,
        "positive_count": 1,
        "positive_rank": rank,
...
def main() -> None:
    ...
    subprocess.run([sys.executable, str(PHASE_BUILDER), "--output", str(fixture)], check=True)
    ...
    for query in manifest["queries"]:
        ...
        source_context = context_at(source, source_anchor)
        ...
        for i, target_time in enumerate(target["downbeats"]):
            ...
            is_positive = abs(target_time - gt_time) <= TOLERANCE
            candidates.append(
                {
        ...
        q_eval = evaluate_query(candidates)
    ...
    report = {
        "schema": "audio-beatthis-loop-ranking/v1",
        "query_count": len(queries),
        "covered_queries": len(covered),
        "candidate_coverage": len(covered) / len(queries),
