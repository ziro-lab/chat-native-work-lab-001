...
public static class CnwlFolderLayoutWin32 {
...
function Get-CnwlWindows {
  $script:rows=@()
  $cb=[CnwlFolderLayoutWin32+EnumWindowsProc]{
    param([IntPtr]$hWnd,[IntPtr]$lParam)
    if([CnwlFolderLayoutWin32]::IsWindowVisible($hWnd)){
      $sb=New-Object System.Text.StringBuilder 1024
      [void][CnwlFolderLayoutWin32]::GetWindowText($hWnd,$sb,$sb.Capacity)
      if($sb.Length -gt 0){$script:rows += [pscustomobject]@{Handle=$hWnd;Title=$sb.ToString()}}
    }
    return $true
  }
  [void][CnwlFolderLayoutWin32]::EnumWindows($cb,[IntPtr]::Zero)
  return $script:rows
}

$env:CNWL_YMM4_FOLDER_LAYOUT_DIR=$OutputDir
...
try{
  for($i=0;$i -lt 180;$i++){
    ...
    foreach($w in (Get-CnwlWindows)){
      if($w.Title -like '*Check for updates*' -or $w.Title -like '*About YukkuriMovieMaker*'){
        [void][CnwlFolderLayoutWin32]::PostMessage($w.Handle,0x0010,[IntPtr]::Zero,[IntPtr]::Zero)
      } elseif($w.Title -eq 'Confirm'){
        [void][CnwlFolderLayoutWin32]::PostMessage($w.Handle,0x0100,[IntPtr]0x0D,[IntPtr]::Zero)
        [void][CnwlFolderLayoutWin32]::PostMessage($w.Handle,0x0101,[IntPtr]0x0D,[IntPtr]::Zero)
      }
    }
  ...
  Remove-Item Env:CNWL_YMM4_FOLDER_LAYOUT_DIR -ErrorAction SilentlyContinue
}