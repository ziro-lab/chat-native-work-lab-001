...
BASELINE_PITCHES = [5.00, 5.08, 5.16, 5.08, 5.00]
RISE_PITCHES = [4.92, 5.04, 5.16, 5.12, 5.08]
FALL_PITCHES = [5.08, 5.12, 5.16, 5.04, 4.92]
...
def phrases_for(text):
    texts = ["エ", "エ", "エ", "エ", "エ"] if text == "エエエエエ" else list(text)
    pitches = BASELINE_PITCHES if len(texts) == 5 else [5.0] * len(texts)
    return [{
...
def synth_shape(raw):
    ...
    try:
        ...
        if phrases:
            ...
            lengths_ok = all(abs(float(m.get("vowel_length", -1)) - 0.12) < 0.000001 for m in moras)
    except Exception:
        pass

    if lengths_ok and close_list(pitches, RISE_PITCHES):
        return make_wav(2900, b"\x03\x00"), "relative-rise", pitches
    if lengths_ok and close_list(pitches, FALL_PITCHES):
        return make_wav(3000, b"\x04\x00"), "relative-fall", pitches
