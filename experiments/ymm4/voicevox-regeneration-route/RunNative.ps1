...
$env:CNWL_VOICEVOX_ROUTE_OUTPUT=$OutputDir
...
try {
  ...
  if($r.schema-ne'cnwl.voicevox-regeneration-route.v1' -or
     $r.status-ne'PASS_VOICEVOX_REGENERATION_ROUTE_INVENTORY' -or
     $r.host-ne'4.56.1.0 Lite' -or
     $r.sourceHead-ne$env:GITHUB_SHA -or $r.error){throw 'Behavior result rejected'}
  $req=@('voicevox_types_found','voicevox_parameter_type_found','voicevox_pronounce_type_found')
  ...
  Write-Output "PASS_VOICEVOX_REGENERATION_ROUTE_INVENTORY: $($req.Count) required assertions"
}
finally {
  if(-not$p.HasExited){Stop-Process -Id $p.Id -Force -ErrorAction SilentlyContinue}
  Remove-Item Env:CNWL_VOICEVOX_ROUTE_OUTPUT -ErrorAction SilentlyContinue
}
