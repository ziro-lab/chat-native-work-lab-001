...
$env:CNWL_VOICEVOX_PUBLIC_ROUTE_OUTPUT=$OutputDir
...
try {
  ...
  if($r.schema-ne'cnwl.voicevox-public-speaker-route.v1' -or
     $r.status-ne'PASS_VOICEVOX_PUBLIC_SPEAKER_ROUTE_INVENTORY' -or
     $r.host-ne'4.56.1.0 Lite' -or
     $r.sourceHead-ne$env:GITHUB_SHA -or $r.error){throw 'Behavior result rejected'}
  $req=@('speaker_type_found','create_voice_async_found','async_state_machine_found','move_next_found')
  ...
  Write-Output "PASS_VOICEVOX_PUBLIC_SPEAKER_ROUTE_INVENTORY"
}
finally {
  if(-not$p.HasExited){Stop-Process -Id $p.Id -Force -ErrorAction SilentlyContinue}
  Remove-Item Env:CNWL_VOICEVOX_PUBLIC_ROUTE_OUTPUT -ErrorAction SilentlyContinue
}
