...
public static class TrackCNavWindow {
...
$env:CNWL_TRACK_C_NAV_DIR=$OutputDir
$p=Start-Process (Join-Path $Ymm4Dir 'YukkuriMovieMaker.exe') -WorkingDirectory $Ymm4Dir -PassThru
try {
 $script:hostPid=$p.Id
 $cb=[TrackCNavWindow+Callback]{
  param([IntPtr]$w,[IntPtr]$unused)
  [uint32]$owner=0
  [void][TrackCNavWindow]::GetWindowThreadProcessId($w,[ref]$owner)
  if($owner -eq $script:hostPid -and [TrackCNavWindow]::IsWindowVisible($w)){
   $s=New-Object System.Text.StringBuilder 1024
   [void][TrackCNavWindow]::GetWindowText($w,$s,1024)
   $title=$s.ToString()
   if($title -like '*Check for updates*' -or $title -like '*About YukkuriMovieMaker*'){
    [void][TrackCNavWindow]::PostMessage($w,0x0010,[IntPtr]::Zero,[IntPtr]::Zero)
   }elseif($title -eq 'Confirm'){
    [void][TrackCNavWindow]::PostMessage($w,0x0100,[IntPtr]0x0D,[IntPtr]::Zero)
    [void][TrackCNavWindow]::PostMessage($w,0x0101,[IntPtr]0x0D,[IntPtr]::Zero)
   }
  }
  return $true
 }
 ...
 while([DateTime]::UtcNow -lt $deadline -and -not $p.HasExited -and -not(Test-Path $result)){
  [void][TrackCNavWindow]::EnumWindows($cb,[IntPtr]::Zero)
  Start-Sleep -Milliseconds 500
 }
 if(-not(Test-Path $result)){
  'status=FAIL_TRACK_C_NAV_TIMEOUT_OR_HOST_EXIT'|Set-Content (Join-Path $OutputDir 'runner-failure.txt')
  throw 'No final navigation result'
 }
 $lines=Get-Content $result
 $lines|Write-Host
 if(-not($lines -contains 'status=PASS_TRACK_C_NAVIGATION')){throw 'Navigation assertions failed'}
 if($lines|Where-Object {$_ -match '=False$'}){throw 'False assertion cannot pass'}
 if(@($lines|Where-Object {$_ -match '=True$'}).Count -lt 30){throw 'Missing navigation coverage'}
} finally {
 if(-not $p.HasExited){Stop-Process -Id $p.Id -Force -ErrorAction SilentlyContinue}
 Remove-Item Env:CNWL_TRACK_C_NAV_DIR -ErrorAction SilentlyContinue
}
