...
$env:CNWL_YMM4_INPUT_DIR=$OutputDir
...
try {
  ...
  $result
  if(-not($result -contains 'status=PASS_TIMELINE_INPUT_ROUTE')){ throw 'Input route probe did not pass' }
  foreach($required in @('item_pointer_route_observed=True','item_reclick_pointer_route_observed=True','blank_pointer_route_observed=True','ruler_pointer_route_observed=True')){
    if(-not($result -contains $required)){ throw "Missing $required" }
  }
} finally {
  if(-not $p.HasExited){ Stop-Process -Id $p.Id -Force -ErrorAction SilentlyContinue }
  Remove-Item Env:CNWL_YMM4_INPUT_DIR -ErrorAction SilentlyContinue
}
