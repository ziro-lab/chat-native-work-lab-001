...
public static class CnwlPatchSurfaceWin32 {
...
function Get-CnwlWindows {
  $script:rows=@()
  $cb=[CnwlPatchSurfaceWin32+EnumWindowsProc]{
    param([IntPtr]$hWnd,[IntPtr]$lParam)
    if([CnwlPatchSurfaceWin32]::IsWindowVisible($hWnd)){
      $sb=New-Object System.Text.StringBuilder 1024
      [void][CnwlPatchSurfaceWin32]::GetWindowText($hWnd,$sb,$sb.Capacity)
      if($sb.Length -gt 0){$script:rows += [pscustomobject]@{Handle=$hWnd;Title=$sb.ToString()}}
    }
    return $true
  }
  [void][CnwlPatchSurfaceWin32]::EnumWindows($cb,[IntPtr]::Zero)
  return $script:rows
}
  ...
  for($i=0;$i -lt 180;$i++){
    ...
    foreach($w in (Get-CnwlWindows)){
      if($w.Title -like '*Check for updates*' -or $w.Title -like '*About YukkuriMovieMaker*'){
        [void][CnwlPatchSurfaceWin32]::PostMessage($w.Handle,0x0010,[IntPtr]::Zero,[IntPtr]::Zero)
      } elseif($w.Title -eq 'Confirm'){
        [void][CnwlPatchSurfaceWin32]::PostMessage($w.Handle,0x0100,[IntPtr]0x0D,[IntPtr]::Zero)
        [void][CnwlPatchSurfaceWin32]::PostMessage($w.Handle,0x0101,[IntPtr]0x0D,[IntPtr]::Zero)
      }
    }
