...
public static class CnwlVqaAudioWin32 {
...
function Get-CnwlWindows {
  $script:rows=@()
  $callback=[CnwlVqaAudioWin32+EnumWindowsProc]{
    param([IntPtr]$hWnd,[IntPtr]$lParam)
    if([CnwlVqaAudioWin32]::IsWindowVisible($hWnd)){
      $sb=New-Object System.Text.StringBuilder 1024
      [void][CnwlVqaAudioWin32]::GetWindowText($hWnd,$sb,$sb.Capacity)
      $title=$sb.ToString()
  ...
  [void][CnwlVqaAudioWin32]::EnumWindows($callback,[IntPtr]::Zero)
  return $script:rows
}
...
try {
  ...
  while([DateTime]::UtcNow-lt$limit -and -not$process.HasExited -and -not(Test-Path $result)){
    foreach($w in (Get-CnwlWindows)){
      ...
      if($w.Title -like '*Check for updates*' -or $w.Title -like '*About YukkuriMovieMaker*'){
        [void][CnwlVqaAudioWin32]::PostMessage($w.Handle,0x0010,[IntPtr]::Zero,[IntPtr]::Zero)
      } elseif($w.Title -eq 'Confirm'){
        [void][CnwlVqaAudioWin32]::PostMessage($w.Handle,0x0100,[IntPtr]0x0D,[IntPtr]::Zero)
        [void][CnwlVqaAudioWin32]::PostMessage($w.Handle,0x0101,[IntPtr]0x0D,[IntPtr]::Zero)
      }
    }
  ...
  foreach($name in @('progress.txt','host-windows.txt','observation.json','ui-text.txt')){
