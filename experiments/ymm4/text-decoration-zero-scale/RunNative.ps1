...
try {
  ...
  if($r.schema-ne'cnwl.text-decoration-tiny-scale.v2' -or
     $r.status-ne'PASS_TEXT_DECORATION_TINY_SCALE' -or
  ...
  $req=@(
    ...
    'tiny_scale_marker_rendered',
    'micro_scale_marker_rendered',
    'visible_marker_adds_width',
    'zero_scale_does_not_collapse_width',
    'tiny_scale_collapses_marker_width',
    'micro_scale_approximately_matches_baseline'
  )
  ...
  Write-Output "PASS_TEXT_DECORATION_TINY_SCALE: $($req.Count) required assertions"
}
finally {
